...
if (!class_exists('WC_SecureTrading_Apple_Gateway')) :
    /**
     * Required minimums and constants
     */
    class WC_SecureTrading_Apple_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
            $this->id = SECURETRADING_APPLE_PAY;
            $this->icon = '';
            $this->has_fields = true;
            $this->method_title = __( 'Trust Payments - Apple Pay', SECURETRADING_TEXT_DOMAIN );
            $this->method_description = __( 'Accept Apple Pay payments via the Trust Payments gateway.', SECURETRADING_TEXT_DOMAIN );
            $this->merchant_name = __( 'Trust Payments Merchant', SECURETRADING_TEXT_DOMAIN );
            ...
            $this->merchant_name = ( $this->get_option( 'merchant_name' ) ) ? $this->get_option( 'merchant_name' ) : $this->merchant_name;
            $this->button_style = $this->get_option('button_style', 'black' );
            ...
            if ( is_admin() ) {
                add_action( 'woocommerce_update_options_payment_gateways_' . SECURETRADING_APPLE_PAY, array( $this, 'process_admin_options' ) );
            }

            // Save order meta
            add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'mng_apple_pay_order_meta' ) );

            // Display transction detail on the order detail
            add_action( 'woocommerce_admin_order_data_after_order_details', array( $this, 'mgn_display_apple_pay_order_meta' ), 10, 1 );

            // Process capture payment
            add_action('woocommerce_order_action_st_apple_capture_payment', array($this, 'mgn_apple_pay_admin_capture_payment'));
            add_action('woocommerce_order_action_st_apple_cancel_payment', array($this, 'mgn_apple_pay_admin_cancel_order'));
        }
        ...
        public function init_gateway_setting() {
            $this->form_fields = require( SECURETRADING_PATH . 'admin/securetrading-apple-pay-settings.php' );
        }
         ...
         * @author        Minh Hung
         */
        public function mng_apple_pay_order_meta( $order_id ) {
            $_payment_method = get_post_meta( $order_id, '_payment_method', true);
            if ( SECURETRADING_APPLE_PAY === $_payment_method ) {
                update_post_meta( $order_id, '_'.SECURETRADING_APPLE_PAY.'_method', 'apple_pay' );
            }
        }
         ...
         * @author        Minh Hung
         */
        public function mgn_display_apple_pay_order_meta($order) {
            /* Transaction detail */
            $_payment_method = $order->get_payment_method();
            $order_id = $order->get_id();
            if ( SECURETRADING_APPLE_PAY === $_payment_method ) {
                $this->_helper->mgn_display_order_transaction_info( $order_id, SECURETRADING_APPLE_PAY );
            }
        }
         ...
         * @return array
         */
        public function mgn_apple_pay_admin_capture_payment($order) {
         ...
         * @return array
         */
        public function mgn_apple_pay_admin_cancel_order($order) {
